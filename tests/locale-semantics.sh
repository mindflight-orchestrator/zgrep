...
if [ "${#locales[@]}" -eq 0 ]; then
    echo 'zgrep locale differentials skipped: no UTF-8 locale available'
    exit 0
fi
...
compare_file() {
    ...
    zgrep_status=$?
    set -e
    sed 's/^grep:/zgrep:/' "$TEST_DIR/gnu.err" >"$TEST_DIR/gnu-normalized.err"
...
printf 'zgrep UTF-8 locale differentials passed (%s cases across %s locales)\n' \
    "$case_count" "${#locales[@]}"
